...
export default function UserDetails() {
  ...
  const toggleBlock = async () => {
    ...
    setUpdating(true);
    try {
      const updated = await Api.patch(`/admin/users/${id}/`, {
        is_blocked: !user.is_blocked,
      });
      setUser(updated.data);
      toast.success(`User ${updated.data.is_blocked ? 'blocked' : 'unblocked'} successfully`);
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          ...
          <div className="flex flex-col items-start md:items-end">
            ...
            <button
              onClick={toggleBlock}
              disabled={updating || user.is_staff}
              title={user.is_staff ? "Cannot block an admin user" : ""}
              className={`mt-3 px-4 py-2 rounded-md text-white text-sm font-medium transition-colors ${
                user.is_blocked 
                  ? "bg-green-600 hover:bg-green-700" 
                  : "bg-red-600 hover:bg-red-700"
              } ${(updating || user.is_staff) ? "opacity-70 cursor-not-allowed" : ""}`}
            >
              {updating ? (
