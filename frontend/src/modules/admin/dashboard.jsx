      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-blue-500 flex items-center mt-2">
                    <FiTrendingUp className="mr-1" /> Per Order
                  </p>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1">
                    {orders.reduce((acc, order) => acc + order.products.reduce((sum, p) => sum + p.quantity, 0), 0)}
                  </p>
                  <p className="text-sm text-green-500 flex items-center mt-2">
                    <FiTrendingUp className="mr-1" /> Volume
                  </p>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="h-80">
                  ...
                  <PieChart>
                    <Pie
                      data={[
                        ...
                        { 
                          name: "Pending", 
                          value: orders.filter(o => ["CREATED", "PAYMENT_INITIATED", "PAID", "PROCESSING", "SHIPPED"].includes(o.status)).length 
                        },
                        { 
                          name: "Cancelled", 
                ...
                <div className="flex justify-center gap-4 mt-4 text-sm text-gray-600">
                    <div className="flex items-center"><span className="w-3 h-3 bg-green-500 rounded-full mr-1"></span>Completed</div>
                    <div className="flex items-center"><span className="w-3 h-3 bg-yellow-500 rounded-full mr-1"></span>Pending</div>
                    <div className="flex items-center"><span className="w-3 h-3 bg-red-500 rounded-full mr-1"></span>Cancelled</div>
                </div>
              </div>
            </div>
          </div>
                    ...
                    data={
                        ...
                        Object.values(orders.reduce((acc, order) => {
                            order.products.forEach(p => {
                                if (!acc[p.id]) acc[p.id] = { name: p.name, sales: 0 };
                                acc[p.id].sales += p.quantity;
                            });
                            return acc;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Stock Levels */}
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <h3 className="font-bold text-lg mb-6">Low Stock Alert</h3>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stockData} layout="vertical">
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
                    />
                    <Bar dataKey="stock" fill="#EF4444" radius={[0, 4, 4, 0]} /> {/* Red for alert/stock */}
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {recentOrders.length > 0 ? (
                  recentOrders.map((order, idx) => {
                    let statusColor = "bg-yellow-100 text-yellow-800"; // Default: Active/Pending
                    ...
                    return (
                      <div key={order.id || idx} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:shadow-sm transition-shadow">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <p className="font-medium text-gray-800">{order.products?.[0]?.name || "Product Name"}</p>
                            <p className="text-sm text-gray-500">Qty: {order.products?.[0]?.quantity || 1}</p>
                            <p className="text-sm text-gray-500">{order.createdAt.format("ddd, MMM D, YYYY")}</p>
                          </div>
                        </div>
                        <div className="text-right flex flex-col items-end gap-1">
                          <p className="font-medium text-gray-800">${order.totalPrice.toFixed(2)}</p>
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusColor}`}>{order.status}</span>
                        </div>
                      </div>
                    );
                  })
